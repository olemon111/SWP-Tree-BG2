 ...
 */
    ...
    public:
        ...
        ~group()
        {
            if (entry_space) {
                delete [] entry_space;
                dram_space -= nr_entries_ * sizeof(bentry_t);
            }
    ...
    void group::Init(CLevel::MemControl *mem)
    {
        ...
        nr_entries_ = 1;
        // entry_space = (bentry_t *)malloc(nr_entries_ * sizeof(bentry_t));
        entry_space = new bentry_t[nr_entries_];
        dram_space += nr_entries_ * sizeof(bentry_t);
    ...
    void group::bulk_load(std::vector<std::pair<uint64_t, uint64_t>> &data, CLevel::MemControl *mem)
    {
        ...
        // bentry_t *new_entry_space = (bentry_t *)malloc(nr_entries_ * sizeof(bentry_t));
        bentry_t *new_entry_space = new bentry_t[nr_entries_];
        dram_space += nr_entries_ * sizeof(bentry_t);
    ...
    {
        // entry_space = (bentry_t *)malloc(next_entry_count * sizeof(bentry_t));
        entry_space = new bentry_t[next_entry_count];
        dram_space += next_entry_count * sizeof(bentry_t);
    }
    ...
    // int group::linear_search_upper_bound(int m, const uint64_t & key) const {
    //     int bound = 1;
    //     int l, r;  // will do binary search in range [l, r)
    //     if(entry_space[m].entry_key > key) {
    //         int size = m;
    ...
    void group::expand(CLevel::MemControl *mem)
    {
        ...
        // Meticer timer;
        // timer.Start();
        // bentry_t *new_entry_space = (bentry_t *)malloc(next_entry_count * sizeof(bentry_t));
        bentry_t * new_entry_space = new bentry_t[next_entry_count];
        dram_space += next_entry_count * sizeof(bentry_t);
        size_t new_entry_count = 0;
...
//         {
//             clevel_mem_ = new CLevel::MemControl(CLEVEL_PMEM_FILE, CLEVEL_PMEM_FILE_SIZE);
//             group_space = (group *)malloc(groups * sizeof(group));
//             dram_space += groups * sizeof(group);
// #ifdef MULTI_THREAD
...
//         }
        ...
        ~letree()
        {
            ...
            if (group_space) {
                ...
                delete [] group_space;
                dram_space -= nr_groups_ * sizeof(group);
            }
        ...
        {
            nr_groups_ = 1;
            // group_space = (group *)malloc(sizeof(group));
            group_space = new group[1];
            dram_space += sizeof(group);
#ifdef MULTI_THREAD
    ...
    void letree::ExpandTree()
    {
        ...
        dram_space += new_nr_groups * sizeof(group);
#ifdef MULTI_THREAD
