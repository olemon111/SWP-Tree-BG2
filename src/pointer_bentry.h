...
#define USE_DELETE_0
    ...
    { // without Buncket main key
        ...
        status PutBufKV(uint64_t new_key, uint64_t value, int &data_index)
        {
            ...
            //   NVM::PMem_persist(pkey(target_idx), suffix_bytes + value_size);
            fence();
    ...
    public:
        ...
        status Load(uint64_t *keys, uint64_t *values, int count)
        {
            ...
            NVM::PMem_persist(this, sizeof(*this));
            return status::OK;
        }
 ...
 */
            ...
            Expand_(CLevel::MemControl *mem, SortBuncket *&next, uint64_t &split_key, int &prefix_len)
    {
        ...
        next->next_bucket = this->next_bucket;
        NVM::PMem_persist(next, sizeof(*next));
    ...
    class __attribute__((aligned(64))) UnSortBuncket
    {
        ...
        status PutBufKV(uint64_t new_key, uint64_t value, int &data_index, bool flush = true, char *fingerprint = nullptr);
    ...
    public:
        ...
        status Expand_(CLevel::MemControl *mem,
                       UnSortBuncket *&next, uint64_t &split_key, int &prefix_len, char *old_finger = nullptr, char *new_finger = nullptr);
        ...
        status Put(CLevel::MemControl *mem, uint64_t key, uint64_t value, char *fingerprint = nullptr);

        status Update(CLevel::MemControl *mem, uint64_t key, uint64_t value, char *fingerprint = nullptr);

        status Get(CLevel::MemControl *mem, uint64_t key, uint64_t &value, const char *fingerprint = nullptr) const;

        status Scan(CLevel::MemControl *mem, uint64_t start_key, int &len, std::vector<std::pair<uint64_t, uint64_t>> &results, bool if_first) const;

        status Delete(CLevel::MemControl *mem, uint64_t key, uint64_t *value, char *fingerprint = nullptr);
    ...
    status UnSortBuncket<bucket_size, value_size, key_size, max_entry_count>::
            PutBufKV(uint64_t new_key, uint64_t value, int &data_index, bool flush, char *fingerprint)
    {
        ...
        if (flush) {
            clflush((void *)&records[data_index]);
#ifdef TEST_PMEM_SIZE
            NVM::pmem_size += CACHE_LINE_SIZE;
#endif
            fence();
    ...
    status UnSortBuncket<bucket_size, value_size, key_size, max_entry_count>::
            Expand_(CLevel::MemControl *mem,
                    UnSortBuncket *&next, uint64_t &split_key, int &prefix_len, char *old_finger, char *new_finger)
    {
        // int expand_pos = entries / 2;
        ...
        {
            // next->Put(nullptr, key(i), value(i));
            next->PutBufKV(key(sorted_index_[i]), value(sorted_index_[i]), idx, false, new_finger);
            idx++;
            // next->entries++;
            // clflush(&next->header);
        }
    ...
    template <const size_t bucket_size, const size_t value_size, const size_t key_size,
        const size_t max_entry_count>
    status UnSortBuncket<bucket_size, value_size, key_size, max_entry_count>::
            Put(CLevel::MemControl *mem, uint64_t key, uint64_t value, char *fingerprint)
    {
        status ret = status::OK;
        int idx = entries;
        // Common::timers["CLevel_times"].start();
        ret = PutBufKV(key, value, idx, true, fingerprint);
        if (ret != status::OK)
        {
            return ret;
        }
    ...
    template <const size_t bucket_size, const size_t value_size, const size_t key_size,
        const size_t max_entry_count>
    status UnSortBuncket<bucket_size, value_size, key_size, max_entry_count>::
            Update(CLevel::MemControl *mem, uint64_t key, uint64_t value, char *fingerprint)
    {
        bool find = false;
        int pos = Find(key, find, fingerprint);
        if (!find)
    ...
    template <const size_t bucket_size, const size_t value_size, const size_t key_size,
        const size_t max_entry_count>
    status UnSortBuncket<bucket_size, value_size, key_size, max_entry_count>::
            Get(CLevel::MemControl *mem, uint64_t key, uint64_t &value, const char *fingerprint) const
    {
        bool find = false;
        int pos = Find(key, find, fingerprint);
        if (!find)
    ...
    template <const size_t bucket_size, const size_t value_size, const size_t key_size,
        const size_t max_entry_count>
    status UnSortBuncket<bucket_size, value_size, key_size, max_entry_count>::
            Delete(CLevel::MemControl *mem, uint64_t key, uint64_t *value, char *fingerprint)
    {
...
#ifdef USE_FINGER
        auto ret = (entrys[pos].pointer.pointer(mem->BaseAddr()))->Update(mem, key, value, entrys[pos].fingerprint);
#else
        auto ret = (entrys[pos].pointer.pointer(mem->BaseAddr()))->Update(mem, key, value, nullptr);
#endif
...
#ifdef USE_FINGER
        auto ret = (entrys[pos].pointer.pointer(mem->BaseAddr()))->Get(mem, key, value, (const char*)entrys[pos].fingerprint);
#else
        auto ret = (entrys[pos].pointer.pointer(mem->BaseAddr()))->Get(mem, key, value, nullptr);
#endif
        // if(ret != status::OK) {
...
#ifdef USE_FINGER
        auto ret = (entrys[pos].pointer.pointer(mem->BaseAddr()))->Delete(mem, key, value, entrys[pos].fingerprint);
#else
        auto ret = (entrys[pos].pointer.pointer(mem->BaseAddr()))->Delete(mem, key, value, nullptr);
#endif
        return ret == status::OK;
    }
...
#ifdef USE_FINGER
        auto ret = (entrys[pos].pointer.pointer(mem->BaseAddr()))->Put(mem, key, value, entrys[pos].fingerprint);
#else 
        auto ret = (entrys[pos].pointer.pointer(mem->BaseAddr()))->Put(mem, key, value, nullptr);
#endif
        // if(ret == status::Full){
...
#ifdef USE_FINGER
            (entrys[pos].pointer.pointer(mem->BaseAddr()))->Expand_(mem, next, split_key, prefix_len, entrys[pos].fingerprint, entrys[pos+1].fingerprint);
#else
            (entrys[pos].pointer.pointer(mem->BaseAddr()))->Expand_(mem, next, split_key, prefix_len, nullptr, nullptr);
#endif
...
// #endif
    ...
    {
        if (left->buf.entries == PointerBEntry::pb_entry_count)
        {
            ...
            clflush(right);
            fence();
            // NVM::PMem_persist(right, sizeof(PointerBEntry));
            left->entrys[left->buf.entries - 1].SetInvalid();
            left->buf.entries -= 1;
            clflush(left);
            fence();
            // NVM::PMem_persist(left, sizeof(PointerBEntry));
        }
        else
        {
