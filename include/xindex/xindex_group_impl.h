...
namespace xindex
{
  ...
  template <class key_t, class val_t, bool seq, size_t max_model_n>
  double Group<key_t, val_t, seq, max_model_n>::
      group_size()
  {
    // std::cout << model_n << " model: " << sizeof(models) << " , " << size(models) << std::endl;
    // std::cout << array_size << "arr: " << sizeof(data) << " , " << array_size * sizeof(record_t) << std::endl;
    // return sizeof(models) + array_size * sizeof(record_t);
    return sizeof(models) + array_size * sizeof(record_t) + buffer->size() + buffer_temp->size();
    // return 0;
  }
