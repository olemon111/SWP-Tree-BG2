...
namespace xindex
{
                                                         ...
                                                         leaf_t *target)
  {
    ...
    while (true)
    {
      ...
      while (true)
      {
        ...
        parent_ptr->lock();
        if (parent_ptr != node_ptr->parent)
        { // when parent is split by others
      ...
      { // recursive split, knock the parent as the new node to split
        ...
        { // insert to the new sibling
          if (slot == mid)
          { // prev split key will be the new split key
  ...
  {
    if (pos < n - 1)
    { // pre-fetched array is available for next read
      pos++;
      has_next = true;
    }
    else
    { // fetch from next leaves
      while (true)
      { // because next leaf might be empty (all logical removed)
        if (next == nullptr)
        {
          has_next = false;
          return;
