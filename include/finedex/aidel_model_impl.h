...
namespace aidel
{
                                         ...
                                         size_t size, size_t _maxErr) : maxErr(_maxErr), capacity(size)
    {
        ...
        sz += sizeof(key_t) * size;
        vals = (val_t *)malloc(sizeof(val_t) * size);
        sz += sizeof(val_t) * size;
        ...
        sz += sizeof(levelbin_type *) * (size + 1);
        for (int i = 0; i < size + 1; i++)
        {
            levelbins[i] = nullptr;
        }
        mobs = (model_or_bin_t **)malloc(sizeof(model_or_bin_t *) * (size + 1));
        sz += sizeof(model_or_bin_t *) * (size + 1);
        for (int i = 0; i < size + 1; i++)
        {
            mobs[i] = nullptr;
        }
    ...
    {
        if (levelbins[0])
            levelbins[0]->self_check();
        for (size_t i = 1; i < capacity; i++)
        {
            assert(keys[i] > keys[i - 1]);
            if (levelbins[i])
                levelbins[i]->self_check();
        }
        if (levelbins[capacity])
            levelbins[capacity]->self_check();
    }
    ...
    template <class key_t, class val_t>
    double AidelModel<key_t, val_t>::model_size()
    {
        return sz;
    }
