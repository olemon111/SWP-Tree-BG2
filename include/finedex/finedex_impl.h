...
namespace aidel
{
    ...
    template <class key_t, class val_t>
    double FINEdex<key_t, val_t>::index_size()
    {
        double size = model_keys.size() * sizeof(key_t) + aimodels.size() * sizeof(aidelmodel_type) + segments.size() * sizeof(canonical_segment);
        for (size_t i = 0; i < aimodels.size(); i++)
        {
            aidelmodel_type aimodel = aimodels[i];
            // cout << "size: " << aimodel.model_size() << endl;
            size += aimodel.model_size();
        }
        return size;
    }
