...
namespace aidel
{
    ...
    private:
        result_t con_insert_start(const key_type &key, const data_type &value)
        {
            ...
            if (m_root)
            {
                ...
                { // insert into current child and full
                    // need retrain?
                    if (root->isfull())
                    {
            ...
            { // only occors when there is one bin
                uint64_t child_ver;
                child_bin *child = m_headbin;
                if (child == nullptr)
                {
        ...
        result_t con_insert_start_retrain(const key_type &key, const data_type &value)
        {
            ...
            { // only occors when there is only one bin
                memory_fence();
                if (m_headbin == NULL)
                {
                    m_headbin = m_tailbin = allocate_childbin();
    ...
    public:
        result_t remove(const key_type &key)
        {
            ...
            { // occurs only when there is only one bin
