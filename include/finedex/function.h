...
typedef uint64_t f_key_type;
typedef uint64_t val_type;

std::vector<f_key_type> exist_keys;
std::vector<f_key_type> non_exist_keys;
...
template <typename f_key_type>
std::vector<f_key_type> read_timestamp(const char *path);
template <typename f_key_type>
std::vector<f_key_type> read_document(const char *path);
...
void normal_data()
{
    ...
    for (size_t i = 0; i < Config.exist_num; ++i)
    {
        f_key_type a = rand_normal(gen) * 1000000000000;
        if (a < 0)
        {
            i--;
            continue;
...
void lognormal_data()
{
    ...
    for (size_t i = 0; i < Config.exist_num; ++i)
    {
        f_key_type a = rand_lognormal(gen) * 1000000000000;
        assert(a > 0);
        exist_keys.push_back(a);
    }
    if (Config.insert_ratio > 0)
...
void timestamp_data()
{
    ...
    // std::vector<f_key_type> read_keys = read_timestamp<f_key_type>("../FINEdex2/workloads/timestamp.sorted.200M");
    std::vector<f_key_type> read_keys = read_timestamp<f_key_type>("osm/osm_cellids_800M_uint64");
...
{
    // assert(Config.read_ratio == 1);
    std::vector<f_key_type> read_keys = read_timestamp<f_key_type>("../FINEdex2/workloads/document-id.sorted.10M");
    size_t size = read_keys.size();
...
void skew_data()
{
    f_key_type maxdata = 987654321;
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<f_key_type> rand_int(0, 987654321);
    ...
    f_key_type upbound = (f_key_type)(Config.skewness * maxdata);
    COUT_THIS("[skewness upbound: ]" << upbound);
    std::uniform_int_distribution<f_key_type> rand_skew(0, upbound);
    non_exist_keys.reserve(Config.item_num);
    for (size_t i = 0; i < Config.item_num; ++i)
    {
...
std::vector<f_key_type> read_timestamp(const char *path)
{
    std::vector<f_key_type> vec;
    FILE *fin = fopen(path, "rb");
    f_key_type buf[BUF_SIZE];
    while (true)
    {
        size_t num_read = fread(buf, sizeof(f_key_type), BUF_SIZE, fin);
        for (int i = 0; i < num_read; i++)
        {
            vec.push_back(buf[i]);
        }
